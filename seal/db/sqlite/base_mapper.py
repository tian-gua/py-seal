...
class BaseMapper(metaclass=abc.ABCMeta):
    ...
    def first(self, **conditions):
        ...
        try:
            c = conn.cursor()
            result = c.execute(sql, tuple([val[0] for _, *val in conditions]))
            row = result.fetchone()
            if row is None:
                return None
    ...
    def update(self, sets: _list[tuple], conditions: _list[tuple]):
        ...
        try:
            c = conn.cursor()
            c.execute(sql, tuple([val[0] for _, *val in sets] + [val[0] for _, *val in conditions]))
            conn.commit()
        finally:
            conn.close()
    ...
    def delete(self, **conditions):
        ...
        try:
            c = conn.cursor()
            c.execute(sql, tuple([val[0] for _, *val in conditions]))
            conn.commit()
        finally:
            conn.close()
